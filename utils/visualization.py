...
def _apply_style():
    """Apply the project's default plot style. Call explicitly from notebooks
    if you want the whitegrid look; importing this module no longer mutates
...
def plot_confusion_matrix(cm: np.ndarray,
    ...
    if normalize:
        ...
        # Avoid divide-by-zero / NaN for rows with no true instances.
        safe_sums = np.where(row_sums == 0, 1.0, row_sums)
        cm = cm / safe_sums[:, np.newaxis]
        # Restore zeros where the row had no true instances (instead of NaN).
        cm[row_sums == 0] = 0.0
